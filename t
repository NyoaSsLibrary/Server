...
local favoriteConfigFile = configFolder .. "/favorite.txt"
local favoriteConfig = isfile(favoriteConfigFile) and readfile(favoriteConfigFile) or nil

local autoLoadEnabled = true

local function loadFavoriteConfig()
    if not autoLoadEnabled or not favoriteConfig then return end
    local autoLoadFile = configFolder .. "/" .. favoriteConfig .. ".json"
    if isfile(autoLoadFile) then
        ...
        if success then
            ...
            createNotify("Auto-loaded favorite config '" .. favoriteConfig .. "'")
        end
    end
end
...
local function loadConfigs()
    ...
    for _, file in ipairs(configs) do
        ...
        if configName then
            ...
            star.Position = UDim2.new(1, -30, 0.5, -10)
            star.BackgroundTransparency = 1
            star.Text = configName == favoriteConfig and "⭐" or ""
            ...
            btn.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    if pressDuration >= 2 then
                        favoriteConfig = configName
                        writefile(favoriteConfigFile, configName)
                        createNotify(configName .. " set as favorite!")
                        star.Text = "⭐"
                    else
...
createToggle(pages.Setting, "Auto Load", function(state)
